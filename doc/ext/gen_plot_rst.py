...
def generate_example_rst(app):
    ...
    fhindex = file(os.path.join(root_dir, 'index.%s' % EXT), 'w')
    fhindex.write("""\
...
def generate_dir_rst(dir, fhindex, example_dir, root_dir, plot_gallery):
    ...
    if not dir == '.':
        ...
        target_dir = root_dir
        src_dir = example_dir
    if not os.path.exists(os.path.join(src_dir, 'README.%s' % EXT)):
        print src_dir
        print 80*'_'
        print ('Example directory %s does not have a README.%s file'
                        % (src_dir, EXT))
...
""" % file(os.path.join(src_dir, 'README.%s' % EXT)).read())
...
def generate_file_rst(fname, target_dir, src_dir, plot_gallery):
    ...
    f = open(os.path.join(target_dir, fname[:-2] + EXT),'w')
